...
for slovo in text.split():
    text_upraven.append(slovo.strip(".@?!,").lower())

for slovo in text_upraven:
    if slovo not in vyskyt_slov:
        vyskyt_slov.setdefault(slovo,1)
    else:
        vyskyt_slov[slovo] += 1

nejcastejsi = sorted(list(vyskyt_slov.values()), reverse= True) [:9]

vysledky = []

pprint.pprint(nejcastejsi)
pprint.pprint(vyskyt_slov)
for vyskyt in vyskyt_slov:
    if vyskyt_slov[vyskyt] in nejcastejsi:
        vysledky.append((vyskyt_slov[vyskyt], vyskyt))

for index, tuple in enumerate(sorted(vysledky,reverse= True),1):
    print(
        oddelovac,
        f"|{index}.|{tuple[1]: ^15}|{tuple[0]}x|",
        oddelovac,
        sep="\n"
    )
